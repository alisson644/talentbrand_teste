 require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to test the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator. If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails. There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.

RSpec.describe "/annotations", type: :request do
  
  # Annotation. As you add validations to Annotation, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    skip("Add a hash of attributes valid for your model")
  }

  let(:invalid_attributes) {
    skip("Add a hash of attributes invalid for your model")
  }

  describe "GET /index" do
    it "renders a successful response" do
      Annotation.create! valid_attributes
      get annotations_url
      expect(response).to be_successful
    end
  end

  describe "GET /show" do
    it "renders a successful response" do
      annotation = Annotation.create! valid_attributes
      get annotation_url(annotation)
      expect(response).to be_successful
    end
  end

  describe "GET /new" do
    it "renders a successful response" do
      get new_annotation_url
      expect(response).to be_successful
    end
  end

  describe "GET /edit" do
    it "render a successful response" do
      annotation = Annotation.create! valid_attributes
      get edit_annotation_url(annotation)
      expect(response).to be_successful
    end
  end

  describe "POST /create" do
    context "with valid parameters" do
      it "creates a new Annotation" do
        expect {
          post annotations_url, params: { annotation: valid_attributes }
        }.to change(Annotation, :count).by(1)
      end

      it "redirects to the created annotation" do
        post annotations_url, params: { annotation: valid_attributes }
        expect(response).to redirect_to(annotation_url(Annotation.last))
      end
    end

    context "with invalid parameters" do
      it "does not create a new Annotation" do
        expect {
          post annotations_url, params: { annotation: invalid_attributes }
        }.to change(Annotation, :count).by(0)
      end

      it "renders a successful response (i.e. to display the 'new' template)" do
        post annotations_url, params: { annotation: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "PATCH /update" do
    context "with valid parameters" do
      let(:new_attributes) {
        skip("Add a hash of attributes valid for your model")
      }

      it "updates the requested annotation" do
        annotation = Annotation.create! valid_attributes
        patch annotation_url(annotation), params: { annotation: new_attributes }
        annotation.reload
        skip("Add assertions for updated state")
      end

      it "redirects to the annotation" do
        annotation = Annotation.create! valid_attributes
        patch annotation_url(annotation), params: { annotation: new_attributes }
        annotation.reload
        expect(response).to redirect_to(annotation_url(annotation))
      end
    end

    context "with invalid parameters" do
      it "renders a successful response (i.e. to display the 'edit' template)" do
        annotation = Annotation.create! valid_attributes
        patch annotation_url(annotation), params: { annotation: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "DELETE /destroy" do
    it "destroys the requested annotation" do
      annotation = Annotation.create! valid_attributes
      expect {
        delete annotation_url(annotation)
      }.to change(Annotation, :count).by(-1)
    end

    it "redirects to the annotations list" do
      annotation = Annotation.create! valid_attributes
      delete annotation_url(annotation)
      expect(response).to redirect_to(annotations_url)
    end
  end
end
